...
<task type="auto">
  ...
  <action>
    ...
    **2. Quick Facts Box**
    ...
    - Component Count: **51 components** total (reference 30-COMPONENT-INVENTORY.md)
    - Estimated Effort: 8-12 weeks (1 senior developer)
    - Recommendation: **DO NOT MIGRATE** (unless compelling business need)

    NOTE: Explicitly include the phrase "51 components" in the Quick Facts section for cross-reference verification.
...
<task type="auto">
  ...
  <action>
    ...
    **9. Risk Checkpoints**
    Reference findings from 30-RISK-ASSESSMENT.md for Risk-aware phase gates:

    | Week | Checkpoint | Risk Assessment Reference | Decision |
    |------|------------|---------------------------|----------|
    | 2 | Phase 1 complete | Risk: CSS isolation loss | Proceed or abort |
    | 4 | Charts working | Risk: Performance regression | Proceed or abort |
    | 6 | Composites done | Risk: Event system mismatch | Proceed or abort |
    | 8 | Dashboard stable | Risk: Scope creep | Proceed or abort |
    | 10 | Integration complete | Risk: Feature regression | Final testing or abort |
    | 12 | All tests pass | Risk: All mitigations verified | Deploy or rollback |

    NOTE: Each checkpoint explicitly references "Risk" from risk assessment for verification.
