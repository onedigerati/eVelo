...
Baseline verification (run after execution):
1. Run baseline capture: npm run test:e2e:charts -- --capture
2. Verify server starts and simulation runs (may take 30-60 seconds)
3. Verify charts are detected (visible with data)
4. Check screenshots exist:
   - test/e2e/screenshots/baseline/dashboard-top.png
   - test/e2e/screenshots/baseline/dashboard-bottom.png
5. Review screenshots show charts with actual data (not empty placeholders)
6. (Optional) Run verify mode: npm run test:e2e:charts
